...
@Component({
    selector: 'jhi-driver-drn',
    templateUrl: './driver-drn.component.html'
})
export class DriverDrnComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.driverService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<DriverDrn[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/driver-drn'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/driver-drn', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: DriverDrn) {
        return item.id;
    }
