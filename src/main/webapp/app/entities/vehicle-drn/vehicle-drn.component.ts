...
@Component({
    selector: 'jhi-vehicle-drn',
    templateUrl: './vehicle-drn.component.html'
})
export class VehicleDrnComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.vehicleService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<VehicleDrn[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/vehicle-drn'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/vehicle-drn', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: VehicleDrn) {
        return item.id;
    }
